...
app.get('/postgistest', function (req,res) {
	pool.connect(function(err,client,done) {
		...
		client.query('SELECT region FROM "Regions"',function(err,result) {
		done();
			if(err){
				console.log(err);
				res.status(400).send(err);
      ...
      res.send(req.body);
       ...
       const client = new pg.Client(config);
		...
		client.connect((err)=>{
			...
			var querystring = "INSERT into questionform (question, q1, q2, q3, q4, correct_answer, geom) values ('" + req.body.question +"','" + req.body.q1 +"','" + req.body.q2 +"','" + req.body.q3+"','" + req.body.q4 +"','" + req.body.correct_answer +"'," +"st_geomfromtext('POINT("+req.body.lng+" "+req.body.lat+")', 4326))";
